...
> 반복되는 Excel 변환 규칙을 workbook template 안에 넣어 둡니다.
...
xl3는 반복되는 Excel 변환 규칙을 workbook 파일로 옮겨 두는 TypeScript
라이브러리입니다.

프레임은 단순합니다.

- 누가: 코드를 읽지 않아도 되는 실무자와 운영자
- 무엇을: 반복되는 Excel 변환 규칙
- 어떻게: template workbook, `source_table`, 익숙한 Excel 함수로

```text
raw.xlsx        (입력 데이터)
       +
template.xlsx   (workflow 계약서)
       ↓
result.xlsx     (완성된 workbook)
```

개발자는 엔진을 코드로 소유합니다. 실무자는 파일 기반 흐름을 사용합니다.
raw Excel을 올리고, 승인된 template을 선택하고, 완성된 workbook을 내려받으면
됩니다.
...
- **파일 기반 흐름.** raw `.xlsx`와 승인된 템플릿을 넣으면 완성된 workbook이
  나옵니다.
...
| 접근 | 잘하는 것 | tradeoff |
|---|---|---|
| **xl3** | 실무자가 raw `.xlsx`를 올리고 완성된 workbook을 내려받는 파일 기반 Excel 변환 엔진을 만들기. 업무 규칙은 `template.xlsx` 안에 남습니다. | Alpha입니다. XTL surface는 의도적으로 작고 아직 진화 중입니다. |
| Python/VBA script | 기존 스프레드시트에 가까운 빠른 일회성 자동화. | 업무 규칙이 코드나 특정 담당자의 기억에 남기 쉬워 검토와 인수인계가 어려워집니다. |
| Power Query / Office Scripts / Power Automate | Microsoft 365 안에서 데이터 정리, 작업 자동화, workflow 연결. | 플랫폼 적합성은 강하지만 workflow가 tenant, 계정, 환경에 묶이고 portable workbook 산출물로 남기 어렵습니다. |
| SheetJS, ExcelJS, Aspose.Cells 같은 spreadsheet SDK | workbook을 세밀하게 읽고 쓰는 저수준/고기능 프로그래밍. | 보고서별 규칙을 개발자가 application code 안에 직접 넣는 경우가 많습니다. |
| JXLS, xltpl 같은 template/report engine | spreadsheet-like template 기반 서버사이드 보고서 생성. | 유용하지만 언어/runtime에 묶이는 경우가 많고, 실무자용 브라우저 흐름과 workbook 단위 인수인계가 중심 제품 형태는 아닙니다. |
| Plumsail, Formstack, Conga 같은 문서 생성 SaaS | 관리형 문서 workflow, integration, approval, delivery. | 규칙이 vendor service 안에 남고, self-host 가능한 portable workbook template이 중심은 아닙니다. |
...
템플릿의 숨김 `_config` sheet에서 raw 파일의 source table을 지정할 수 있습니다.
