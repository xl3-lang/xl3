...
0.8.0 의 데이터 블록 정리까지 포함하면 ADR 70개, fixture 154개가
전부 green 입니다. 언어 표면은 early adopter가 시도해볼 만큼 안정됐습니다.
**지금 가장 가치 있는 기여는
실제 사용 후 피드백**입니다 — 1.0 blocker는 [ROADMAP.md](./ROADMAP.md),
의사결정 방식은 [GOVERNANCE.md](./GOVERNANCE.md) 참고.

**0.7.0 → 0.8.0 주요 변경** (2026년 5월): 데이터 블록이
**컬럼 스코프 (column-scoped)** 방식으로 바뀌었습니다 (ADR-0066).
시트 위에 있는 모든 `{{ ... }}` 마커의 외접 사각형을 잡고, 거기에
맞닿은 비어 있지 않은 셀까지 이어 붙여서 블록의 컬럼 범위로 봅니다.
그 범위 바깥에 있는 셀은 — 사이드 요약표, 헤더 컬럼, 오른쪽의
메모 등 — 블록이 확장돼도 원래 행 위치에 그대로 남기 때문에 더 이상
행 증가에 밀려나지 않습니다. 덕분에 오래 묵었던 두 버그가
구조적으로 닫혔습니다 — shared-formula owner 가 중복 복제돼 셀이
조용히 사라지던 #46, 그리고 옆으로 밀린 사이드 셀의 수식 참조가
어긋나던 #47.

0.8.0 은 명시적 **`@block`** 디렉티브 (ADR-0067) 도 추가합니다.
세 가지 형태로 쓸 수 있습니다:

- `{{ @block }}` — 인자 없음. 컬럼 범위는 마커 위치에서 자동 추론
- `{{ @block A:D }}` — 컬럼 범위를 직접 지정
- `{{ @block A2:D7 }}` — 행 × 열 직사각형을 직접 지정

`@block` 을 쓰는 시트에는 엄격(strict) 멀티 블록 판정이 적용됩니다
(ADR-0068) — 모든 `[Column]` 마커가 어떤 블록 안에 속해야 하고,
블록 사각형끼리는 겹칠 수 없습니다. 다른 디렉티브는 가장 가까이서
겹치는 블록에 자동으로 묶입니다 (ADR-0069). **하위 호환:**
`@block` 도 없고 블록 바깥 컬럼에 내용도 없는 템플릿은 0.7.x 와
완전히 동일하게 렌더됩니다. `@block` 은 옵션(opt-in) 기능입니다.
...
[English](./README.md) · **한국어** · [日本語](./README.ja.md) · [简体中文](./README.zh-CN.md) · [繁體中文](./README.zh-TW.md) · [Español](./README.es.md) · [Website](https://xl3.io) · [Spec](./spec) · [Implementations](./IMPLEMENTATIONS.md) · [Roadmap](./ROADMAP.md) · [Governance](./GOVERNANCE.md)
