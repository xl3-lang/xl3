...
### "셀에 `₩1,234,567`을 표시하고 싶은데 `TEXT([Amount], "₩#,##0")`이 안 돼요"
...
| 2. 같은 셀에 `{{ [Amount] }}` (순수 숫자만)을 입력 | XTL 치환 |
...
- **전체 열 참조**를 footer에 두기: `=SUM(B:B)` (또는 `@filter`로 위쪽에서
  데이터 행만 남기기).
- **XTL 집계 함수**: footer 셀에 `{{ SUM([Amount]) }}` 를 두기. 렌더 시점에
  계산되어 숫자로 들어갑니다.
...
```text
{{ IF(ISBLANK([Memo]), "(없음)", [Memo]) }}
```

같은 결과를 fallback 형태로도 쓸 수 있습니다: `IFEMPTY([Memo], "(없음)")`.
같은 술어를 검사합니다.
...
| 행별 동적 하이퍼링크 | `{{ HYPERLINK([Url], [Label]) }}` | quoting 지옥으로 비현실적 | **XTL** |
| "이번 달" 행만 필터 | `{{ @filter MONTH([Date]) = MONTH(TODAY()) }}` | (Excel은 렌더 전 필터 불가) | **XTL 전용** |
