...
`"KR 거래명세서"`, 현재 UTC 날짜)을 보게 됩니다. 사용자에게 더 이상 원시
`{{ ... }}` 플레이스홀더가 노출되지 않습니다.
...
- `select` 의 옵션은 `__inputs__` 행 안에서 파이프(`|`)로 구분합니다(예: `Seoul|Busan|Daegu`). 옵션에 없는 값이 들어오면 `xl3/inputs/select-option` 가 발생합니다. 파이프 분리는 셀 템플릿 평가 **후** 동작하므로 `options: {{ __config__[regions] }}` 같은 표현식도 `__config__[regions]` 가 `Seoul|Busan|Daegu` 리터럴 문자열이면 정상 동작합니다.
