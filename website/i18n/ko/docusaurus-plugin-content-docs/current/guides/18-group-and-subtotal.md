...
셀에 `{{ @subtotal SUM([Amount]) }}` 표현식이 들어있는 행은 **소계 행**입니다.
...
"소계"           |              | {{ @subtotal SUM([Amount]) }}
```

소스 3행 (Acme/Widget/100, Beta/Bolt/50, Acme/Gear/200) 으로 렌더링:
...
"고객 소계"    |                  | {{ @subtotal SUM([Amount]) }}
"지역 소계"    |                  | {{ @subtotal SUM([Amount]) }}
```

위쪽 `@subtotal` (고객 소계) 가 안쪽 키 (`[Customer]`) 에 바인딩되고, 아래
`@subtotal` (지역 소계) 가 `[Region]` 에 바인딩됩니다. 둘 다 경계마다 emit 되지만,
공통 경계에서는 안쪽이 먼저 fire 됩니다.

**"가장 바깥 @subtotal 로 만드는 총계" 패턴:** `@group [Customer]` 한 줄 + 두
...
- [ADR-0038 — `@group` 과 `@subtotal` directive](../../spec/decisions/0038-group-and-subtotal.md)
- [`spec/language.md` § "Group + Subtotal"](../../spec/language.md)
- [Cookbook 03 — 집계 함수](./03-aggregates.md) — 그룹화 없이 블록 레벨에서 쓰는
  `SUM` / `COUNT` / `AVERAGE`
- [Cookbook 15 — Directive 조합](./15-directive-composition.md) — 전체 directive
  순서 규칙
