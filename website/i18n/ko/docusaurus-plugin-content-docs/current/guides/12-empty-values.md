...
문자열 `"0"` 과 `"false"` 는 빈 값이 아닙니다. 이런 값을 걸러내려면 명시적으로 비교하세요: `[Amount] != "0"`.
...
```text
{{ IFEMPTY([Amount], "n/a") }}        → 금액이 0 인 행은 "0" (숫자) 그대로
{{ IF([Amount] = 0, "n/a", [Amount]) }} → 금액이 0 인 행은 "n/a"
```

"누락" 과 "0" 둘 다 `n/a` 로 표시하고 싶다면:
...
```text
{{ @filter [Region] != "" }}        ← Region 이 빈 행은 제거
```
...
```text
{{ IF([Region], [Region], "Unknown") }}      → Region 이 "" 이면 "", 아니면 Region
{{ IF([Amount], [Amount], "no data") }}      → Amount 가 0 이거나 비어 있으면 "no data"
```
...
리터럴이 섞여 있는 셀, 예를 들어 `{{ [Amount] }} won` 의 결과는 `" won"` 입니다 (빈 숫자가 빈 문자열로 직렬화되고 그 뒤에 공백이 붙은 형태).
