...
| `__sources__` | 命名外部数据源声明（集合；见 [外部数据源](#external-data-sources)，依 ADR-0012） |
...
## 外部数据源（External Data Sources） {#external-data-sources}
...
上述数据块默认按垂直方向展开 —— 每个源行渲染一行 —— 不需要显式的 `@repeat` 指令（见 [指令](#directives)）。
...
对于每一主源行，引擎会找到**第一**条匹配的被连接行（依 [比较算法](/zh-CN/spec/language#comparison-algorithm)），并渲染该配对。"第一"由被连接源的自然行序定义 —— 在其 `source_table` 范围内自上而下。这是规范性的：当多条被连接行的连接键相等时，两种实现必须（**MUST**）选取同一行进行配对。
...
- `text` —— 透传宿主字符串。非字符串的宿主值通过规范字符串形式（见 [比较与字符串强制转换](/zh-CN/spec/language#comparison-and-string-coercion)）字符串化。
...
- 必须（**MUST**）从输出工作簿中移除。
- 每个单元格按 [比较与字符串强制转换](/zh-CN/spec/language#comparison-and-string-coercion) 转换为规范字符串形式，并 trim Unicode 空白。trim 后为空的单元格（依 [空值](#empty-values)）被跳过。
...
- 单源迭代顺序是 `source_table` 自上而下的读取顺序。在多源数据（见 [外部数据源](#external-data-sources)）情形下，该规则适用于*主*源的行；命名源参与聚合与连接，但不影响输出顺序。

排序稳定性定义于 [`@sort`](/zh-CN/spec/language#sort)：等值排序键保留源顺序。
...
| 种类 | 备注 |
|---|---|
| Missing | 该行不存在源列，或单元格为空。依 [空值](#empty-values) 视为空。 |
...
| 布尔 | Boolean |
| 带缓存结果的公式 | 该结果的种类 |
| 错误单元格（`#N/A`、`#VALUE!`、`#DIV/0!`、…） | Missing（依 [空值](#empty-values)） |
...
混合文本单元格按字符串渲染。模板的数字/日期格式不会强制转换混合文本单元格。依 [空值](#empty-values) 视为空的值（含按 ADR-0053 的六个源侧 Excel 错误哨兵）在其位置贡献 `""`；引擎产出的 `#DIV/0!` 在其位置替换为字面字符串 `"#DIV/0!"`（ADR-0025）。
