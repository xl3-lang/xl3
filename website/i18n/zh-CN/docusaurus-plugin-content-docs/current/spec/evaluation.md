...
对于每一主源行，引擎会找到**第一**条匹配的被连接行（依 [比较算法](/spec/language#comparison-algorithm)），并渲染该配对。"第一"由被连接源的自然行序定义 —— 在其 `source_table` 范围内自上而下。这是规范性的：当多条被连接行的连接键相等时，两种实现必须（**MUST**）选取同一行进行配对。
...
- `text` —— 透传宿主字符串。非字符串的宿主值通过规范字符串形式（见 [比较与字符串强制转换](/spec/language#comparison-and-string-coercion)）字符串化。
...
- 必须（**MUST**）从输出工作簿中移除。
- 每个单元格按 [比较与字符串强制转换](/spec/language#comparison-and-string-coercion) 转换为规范字符串形式，并 trim Unicode 空白。trim 后为空的单元格（依 [空值](#empty-values)）被跳过。
...
排序稳定性定义于 [`@sort`](/spec/language#sort)：等值排序键保留源顺序。
