...
XTL 当前版本为 **0.1**。参考实现以 `@xl3-lang/xl3` 发布 XTL 0.1，包版本与规范版本独立演进。根据 ADR-0079，1.0 延后到完成后的技术契约通过 G24 的 90 天稳定窗口。第二语言移植、外部 ADR 与公开生产案例作为 1.x 采用目标持续公开追踪，但不阻塞 1.0。

1.0 将要冻结的契约已经起草完成；剩余延后是为了技术浸泡，而不是功能或生态完整度。
...
TypeScript 参考实现在 1.0 时冻结以下 19 个运行时导出（截至 0.10.0 为 13 个，0.11.0 新增 `convertJson` / `previewJson`，0.13.0 新增 `VERSION` / `getEngineInfo`，1.0.0-rc.1 新增 `validateSource` / `validateSourceJson`）。新增导出向后兼容；移除或重命名其中任何一个属于 2.0 才允许的变更。
...
- ADR-0040——保留矩阵修订（大纲级别与 CF/DV 范围扩展均已发布；测试用例 171、172）
- ADR-0041——多行单元格文本契约
