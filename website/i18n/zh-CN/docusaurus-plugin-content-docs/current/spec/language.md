...
`{{ ... }}` 模板块内容的形式语法见 [`grammar.ebnf`](./grammar.ebnf) —— 这是面向移植者与工具方的非规范性辅助材料。术语定义见 [`glossary.md`](/zh-CN/spec/glossary)。
...
| 空值（依 [空值](/zh-CN/spec/evaluation#空值empty-values)） | 0 |
| 可解析为有限数字的字符串 | 解析后的数字 |
...
每个操作数按规范字符串形式（见[比较与字符串强制转换](#比较与字符串强制转换)）转为字符串,然后依次拼接。永远成功,不会出现类型错误。
...
用于 `IF()` 与 `@filter`。遵循[比较与字符串强制转换](#比较与字符串强制转换)中定义的算法。类型不一致时回退至规范字符串形式的码点序;不会产生强制转换错误。
...
- 空值（依[空值](/zh-CN/spec/evaluation#空值empty-values)）是空字符串 `""`。
- 布尔值：`TRUE` 或 `FALSE`（大写）。
...
- 布尔 `false`。
- 数字 `0`。
- 依[空值](/zh-CN/spec/evaluation#空值empty-values)为空的值 —— 缺失、`""` 或仅由空白构成的字符串。
...
当第一个参数依[空值](/zh-CN/spec/evaluation#空值empty-values)为空时,返回第二个参数。否则返回第一个参数。
...
函数按工作簿顺序遍历源的各行,并返回第一个其 `lookup_array` 列依[比较算法](#比较算法)等于 `lookup_value` 的行。如果没有匹配行：
...
`COUNT()` 统计行数。`COUNT([field])` 统计 `[field]` 值依[空值](/zh-CN/spec/evaluation#空值empty-values)非空的行数。
