...
`{{ ... }}` 模板块内容的形式语法见 [`grammar.ebnf`](https://github.com/xl3-lang/xl3/blob/main/spec/grammar.ebnf) —— 这是面向移植者与工具方的非规范性辅助材料。术语定义见 [`glossary.md`](/spec/glossary)。
...
| 空值（依 [空值](/spec/evaluation#empty-values)） | 0 |
| 可解析为有限数字的字符串 | 解析后的数字 |
...
- 空值（依[空值](/spec/evaluation#empty-values)）是空字符串 `""`。
- 布尔值：`TRUE` 或 `FALSE`（大写）。
...
- 布尔 `false`。
- 数字 `0`。
- 依[空值](/spec/evaluation#empty-values)为空的值 —— 缺失、`""` 或仅由空白构成的字符串。
...
当第一个参数依[空值](/spec/evaluation#empty-values)为空时,返回第二个参数。否则返回第一个参数。
...
`COUNT()` 统计行数。`COUNT([field])` 统计 `[field]` 值依[空值](/spec/evaluation#empty-values)非空的行数。
