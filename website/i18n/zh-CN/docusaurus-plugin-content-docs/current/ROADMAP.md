...
- **超出 Y/M/D/EOMONTH/EDATE/DATEDIF 之外的日期运算** —— 该家族其余部分按
  [ADR-0019 修订](./spec/decisions/0019-deferred-date-arithmetic.md)延期。
- **区域感知的字符串排序** ——
  [ADR-0020](./spec/decisions/0020-deferred-locale-collation.md)。
- **多表 join、左连接、多行匹配** ——
  [ADR-0014](./spec/decisions/0014-source-joins.md) 的范围外章节。
- **XLOOKUP 通配符 / 近似匹配 / 反向搜索** ——
  [ADR-0013](./spec/decisions/0013-xlookup-cross-source-lookup.md) 的范围外章节。
- **动态图片插入** —— [ADR-0037](./spec/decisions/0037-rejected-dynamic-image-insertion.md)。
- **运行时单元格变更** —— [ADR-0042](./spec/decisions/0042-rejected-runtime-cell-mutation.md)。
...
- **超出 `093` 的跨写入器 Stage 2 fixture** ——
  [ADR-0006](./spec/decisions/0006-stage-2-ooxml-conformance.md) 修订。
...
| 事项 | 如何帮忙 |
|---|---|
| G13 第二实现 ≥ 80% | 参与 [xl3-py](https://github.com/jinyoung4478/xl3-py)，或启动一个新的移植（Rust、Java、Go）。参见 [PORTERS_GUIDE.md](./PORTERS_GUIDE.md)。 |
| G14 外部 ADR | 挑选一个延期项（pivot 表保留、分页符、ADR-0045 切出的函数等），在 `spec/decisions/` 中起草一份 ADR。参见 [GOVERNANCE.md](./GOVERNANCE.md) 中"变更如何进入项目"。GitHub 上以 `good-first-ADR` 标签提供了若干"入门 ADR 草稿"议题。 |
| G15 生产案例 | 在内部使用 xl3，并分享行得通 / 行不通的地方。如果合适，在 [IMPLEMENTATIONS.md](./IMPLEMENTATIONS.md) 中加一行。如果维护者所在公司（Snack24h）发布公开案例研究，也可作为生产参考案例计入。 |
| G17 韩文 cookbook 16+17 国际化 | 翻译最新的两篇配方（其余已完成）。 |
| G8 基准测试 | 在有代表性的模板上运行 `npm run bench`，分享结果。 |
| G10 跨浏览器 | 把 Safari + Firefox 加入 bundle 冒烟测试。 |
| 函数再提案 | 如果你需要某个被 ADR-0045 拒绝的函数，请使用 [`Function re-proposal`](https://github.com/jinyoung4478/xl3/issues/new?template=function-reproposal.md) 议题模板提交。 |
