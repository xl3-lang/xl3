...
2. **在页脚单元格里写原生 Excel `SUMPRODUCT`** ——xl3 按 ADR-0046 原样保留单元格公式。在页脚单元格里直接写 `=SUMPRODUCT(E2:E10000, F2:F10000)`。由于在编写时还不知道渲染后的行数，请使用超出范围（`E2:E10000`）。注意两个页脚陷阱（自列求和；行超出范围导致重复计算）——参见 [LLM 模板编写指南 § Footer pitfalls](https://github.com/xl3-lang/xl3/blob/main/docs/llm-template-authoring.md#footer-pitfall-1--self-column-sum-raises-순환-참조-circular-reference)。
