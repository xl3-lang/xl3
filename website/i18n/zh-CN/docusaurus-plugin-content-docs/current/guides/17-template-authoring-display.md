...
渲染前模板视图：干净（`—`、`0`）。
渲染后输出：真实的值（xl3 按 [ADR-0046](/zh-CN/spec/decisions/0046-cell-formula-preservation) 不改动公式文本；Excel 在打开时重算，外层包装隐于无形）。
...
来自 [ADR-0049](/zh-CN/spec/decisions/0049-template-display-vs-render-output) 的契约：
...
- [ADR-0049 — Template-display vs render-output: intentional asymmetry](/zh-CN/spec/decisions/0049-template-display-vs-render-output)
