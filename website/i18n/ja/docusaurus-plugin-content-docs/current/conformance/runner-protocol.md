...
これらのケースは現在の正規化器では正規化 **されません**。出現した場合は差異として扱われます。根拠については [ADR-0006](/ja/spec/decisions/0006-stage-2-ooxml-conformance) の修正条項を参照してください。
...
リポジトリの [`IMPLEMENTATIONS.md`](/ja/implementations) には、既知の実装とその適合性レベルがリストされています。
