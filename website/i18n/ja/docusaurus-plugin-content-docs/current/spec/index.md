...
フィクスチャ列には最も若い番号のフィクスチャを掲載しています。ADR ↔ フィクスチャの完全な対応表は [`coverage.md`](/conformance/coverage) を参照してください。
...
XTL 0.1 では以下の領域を意図的に実装に委ねています。2 つのポート間でこれらの選択が異なっていても、どちらも非適合（non-conformant）にはなりません。完全なカタログは [ADR-0021](https://xl3.io/spec/decisions/implementation-defined-boundaries) を参照してください。
