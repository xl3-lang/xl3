...
XTL のバージョンは **0.1** です。リファレンス実装は npm 上で `@xl3-lang/xl3` として XTL 0.1 を配布し、パッケージバージョンは仕様とは独立して動きます。ADR-0079 により、1.0 は完成した技術契約が G24 の 90 日安定性ウィンドウを通過するまで延期されます。第二言語ポート、外部 ADR、本番事例は 1.x 採用目標として公開追跡しますが、1.0 はブロックしません。

1.0 で凍結される契約のドラフトは既に書き上がっています。残る延期理由は技術的ソークであり、機能やエコシステムの完全性ではありません。
...
TypeScript リファレンス実装は、1.0 時点で次の 19 個のランタイムエクスポートを凍結します(0.10.0 までで 13 個、0.11.0 で `convertJson` / `previewJson`、0.13.0 で `VERSION` / `getEngineInfo`、1.0.0-rc.1 で `validateSource` / `validateSourceJson` を追加)。新しいエクスポートを追加するのは後方互換ですが、これらのいずれかを削除・改名することは 2.0 でのみ許される変更です。
