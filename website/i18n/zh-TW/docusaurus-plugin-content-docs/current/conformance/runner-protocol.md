...
`inputs` 區塊列出名稱/值對，執行器會把它們以執行階段輸入的形式（依 ADR-0010 的 `_inputs` 工作表）傳給實作。執行器 **MUST** 將這些值轉送到實作的轉換入口。沒有 `_inputs` 工作表的範本會忽略此欄位。
...
下列情況目前的正規化器 **NOT** 進行正規化。若它們出現就會被視為差異。理由見 [ADR-0006](../spec/decisions/0006-stage-2-ooxml-conformance.md) 修訂。
...
repo 中的 [`IMPLEMENTATIONS.md`](../IMPLEMENTATIONS.md) 列出已知實作及其符合性等級。
