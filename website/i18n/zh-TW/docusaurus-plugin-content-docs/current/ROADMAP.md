...
- **超出 Y/M/D/EOMONTH/EDATE/DATEDIF 的日期運算** — 其餘相關函式依 [ADR-0019 修訂](/zh-TW/spec/decisions/0019-deferred-date-arithmetic)延後。
- **語言環境感知的字串排序** — [ADR-0020](/zh-TW/spec/decisions/0020-deferred-locale-collation)。
- **多重 join、left-join、多列匹配** — [ADR-0014](/zh-TW/spec/decisions/0014-source-joins) 的 out-of-scope 章節。
- **XLOOKUP 萬用字元／近似比對／反向搜尋** — [ADR-0013](/zh-TW/spec/decisions/0013-xlookup-cross-source-lookup) 的 out-of-scope 章節。
- **動態插入圖片** — [ADR-0037](/zh-TW/spec/decisions/0037-rejected-dynamic-image-insertion)。
- **執行期儲存格變更** — [ADR-0042](/zh-TW/spec/decisions/0042-rejected-runtime-cell-mutation)。
- **依 ADR-0043 關卡被否決的函式** — 數學擴充、型別判定（除 ADR-0047 的 `ISBLANK` 外）、NOW／WEEKDAY 等、條件式彙總、TEXT() 格式 token 擴充。詳見 [ADR-0045](/zh-TW/spec/decisions/0045-function-batch-rejected)。
- **串流輸出／類 SXSSF。** 延後至 1.1+。**1.0 時改以文件記載記憶體／列硬性限制（G21）。**
- **範本編譯快取 API。** 延後至 1.1+。
- **PDF／HTML 輸出。** 不在範圍內；xl3 是 xlsx 進、xlsx 出。
- **跨寫入器的 Stage 2 測試案例（除 `093` 外）** — 見 [ADR-0006](/zh-TW/spec/decisions/0006-stage-2-ooxml-conformance) 修訂。
...
| 項目 | 如何協助 |
|---|---|
| G13 第二實作 ≥ 80% | 參與 [xl3-py](https://github.com/jinyoung4478/xl3-py)，或自行起手新移植（Rust、Java、Go）。詳見 [PORTERS_GUIDE.md](/zh-TW/porters-guide)。 |
| G14 外部 ADR | 挑一個延後的項目（pivot table 保留、分頁符、ADR-0045 切出來的函式），在 `spec/decisions/` 草擬一份 ADR。詳見 [GOVERNANCE.md](/zh-TW/governance) 的「變更如何進入專案」。GitHub 上提供了一些 `good-first-ADR` 議題作為起手 ADR 雛形。 |
| G15 生產案例 | 在公司內部使用 xl3，分享哪些順利、哪些不順。如果適合，可在 [IMPLEMENTATIONS.md](/zh-TW/implementations) 加一列。維護者本人的雇主（Snack24h）若推出公開案例研究亦符合資格。 |
