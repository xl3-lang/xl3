...
2. **頁尾儲存格寫原生 Excel `SUMPRODUCT`** — xl3 會原樣保留儲存格公式（ADR-0046）。在頁尾儲存格直接寫 `=SUMPRODUCT(E2:E10000, F2:F10000)`。請用偏大的範圍（`E2:E10000`），因為展開後的列數在編寫時未知。注意兩個頁尾陷阱（自欄參照；列超出造成重覆計算） — 見 [LLM 範本編寫指南 § Footer pitfalls](https://github.com/jinyoung4478/xl3/blob/main/docs/llm-template-authoring.md#footer-pitfall-1--self-column-sum-raises-순환-참조-circular-reference)。
...
- [ADR-0043 — Excel-native preference principle](/zh-TW/spec/decisions/0043-excel-native-preference)
- [ADR-0044 — Function batch accepted](/zh-TW/spec/decisions/0044-function-batch-accepted)
- [ADR-0045 — Function batch rejected](/zh-TW/spec/decisions/0045-function-batch-rejected)
- [ADR-0046 — Cell formula preservation contract](/zh-TW/spec/decisions/0046-cell-formula-preservation)
- [ADR-0047 — ISBLANK as IFEMPTY alias](/zh-TW/spec/decisions/0047-isblank-as-ifempty-alias)
