...
[English](./README.md) · [한국어](./README.ko.md) · [日本語](./README.ja.md) · **简体中文** · [繁體中文](./README.zh-TW.md) · [Español](./README.es.md) · [Website](https://xl3.io) · [Spec](./spec) · [LLM authoring guide](./docs/llm-template-authoring.md) · [Implementations](./IMPLEMENTATIONS.md) · [Roadmap](./ROADMAP.md) · [Governance](./GOVERNANCE.md)
...
**0.7.0 → 0.8.0**(2026 年 5 月):数据块现在采用 **按列界定
(column-scoped)** 的形态(ADR-0066)。侧边汇总表、表头列、右
侧备注等单元格在数据块展开时保留原本的行位置。修复了两个长期存
在的缺陷(#46 shared-formula owner 重复、#47 被横向推开的侧边
单元格公式引用错位)。新增显式的 **`@block`** 指令(ADR-0067),
共三种形式 —— 不带参数、列范围、矩形 —— 并对启用该指令的工作表
加入严格的多块检测(ADR-0068)。**向后兼容:** 没有 `@block`、
且块外侧列也没有内容的模板,渲染结果与 0.7.x 完全一致;
`@block` 是可选(opt-in)特性。

**0.6.0 → 0.7.0**:一组 15 个 ADR(ADR-0051..0065)关闭了所有
残余的语法冲突面 —— 也就是同一种模板写法可能被解析成两种含义、
或者被静默忽略的位置。对用户最显眼的变化是 **聚合函数参数形态**
(ADR-0059):`SUM`、`AVERAGE`、`MIN`、`MAX` 以及单参数的
`COUNT` 现在只接受单列引用,且会在解析阶段拒绝逐行算术。请改用
上游辅助列,或在底部单元格中直接写原生 `=SUMPRODUCT(...)`(详见
[Cookbook 03](./docs/guides/03-aggregates.md))。

**0.5.x → 0.6.0**:原生支持合并单元格表头(ADR-0033)—— 这是
韩国厂商样表(거래명세서 / 정산서 / 발주서)中非常常见的样式,
中国 B2B 模板(对账单、结算单、采购订单)中也有类似形态;数据行
里的合并单元格会广播到所有从单元格(ADR-0035);新增了一份覆盖
图片、条件格式、定义名称、冻结窗格、工作表保护、数据验证与单元
格批注的规范化保留矩阵(ADR-0036);并加入 **`@group` /
`@subtotal`**,可以在同一个数据块内交错生成按客户、按月的小计行
(ADR-0038)。
...
- **Alpha 阶段。** XTL 处于 spec 0.1(draft)。行为正在快速稳定,
  但语言表面在 1.0 之前仍可能调整。
- **单一维护者。** 目前还没有生产环境的参考案例。如果你把 xl3
  用在了关键的地方,目前最有价值的贡献就是 *告诉我* —— 哪怕只是
  开一个 issue 或 discussion,留一句“我们试过了,能用”也好。这
  类反馈,决定了 1.0 是契合真实工作流,还是只契合我对它们的想
  象。
- **70 个 ADR、154 条 conformance fixture,全部通过。** 语言表面
  对早期使用者来说已经足够稳定。
- **MIT、TypeScript、Node ≥ 20.12,浏览器可用。**

1.0 的阻塞项参见 [ROADMAP.md](./ROADMAP.md),决策方式参见
[GOVERNANCE.md](./GOVERNANCE.md)。

> **正在用 LLM 编写 xl3 模板?** 请先阅读
> [`docs/llm-template-authoring.md`](./docs/llm-template-authoring.md)
> —— 它讲清了 LLM 几乎每次都会犯的一个错误(残留的样式行污染每
> 一份输出)以及如何避开。该文档保持英文,因为它是 LLM 直接参考
> 的资料。
