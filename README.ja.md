...
[English](./README.md) · [한국어](./README.ko.md) · **日本語** · [简体中文](./README.zh-CN.md) · [繁體中文](./README.zh-TW.md) · [Español](./README.es.md) · [Website](https://xl3.io) · [Spec](./spec) · [LLM authoring guide](./docs/llm-template-authoring.md) · [Implementations](./IMPLEMENTATIONS.md) · [Roadmap](./ROADMAP.md) · [Governance](./GOVERNANCE.md)
...
**0.7.0 → 0.8.0** (2026 年 5 月): データブロックが **列スコープ**
方式になりました (ADR-0066)。サイド集計表、ヘッダ列、右側のメモは、
ブロックが展開しても元の行位置にそのまま残ります。長年の 2 つの
不具合 (#46 shared-formula owner の重複、#47 横に押し出されたサイドセル
の数式参照がズレる) を解消しました。さらに、明示的な **`@block`**
ディレクティブ (ADR-0067) を 3 形式 — 引数なし、列範囲指定、矩形指定 —
で追加し、オプトインしたシートでは厳格なマルチブロック判定 (ADR-0068)
が有効になります。**後方互換性:** `@block` がなくブロック外側の列にも
内容がないテンプレートは 0.7.x と完全に同じ結果を返します。`@block` は
オプトイン機能です。

**0.6.0 → 0.7.0**: 15 件の ADR (ADR-0051..0065) によって、構文衝突の
余地 — 同じテンプレート形状が二通りに解釈される、あるいは黙って流れて
しまう箇所 — をすべて閉じました。ユーザーへの影響が最も大きい変更は
**集計関数の引数形状** (ADR-0059) で、`SUM`、`AVERAGE`、`MIN`、`MAX`、
1 引数の `COUNT` は列参照 1 つのみを受け付け、行ごとの算術式はパース
時点で拒否されます。回避策は、元データ側にヘルパー列を追加するか、
フッターセルにネイティブの `=SUMPRODUCT(...)` を入れることです (詳細は
[Cookbook 03](./docs/guides/03-aggregates.md) を参照)。

**0.5.x → 0.6.0**: ネイティブな結合ヘッダーセルへの対応 (ADR-0033) —
日本/韓国のベンダー向け帳票 (請求書、納品書、見積書、発注書、거래명세서、
정산서、발주서) でよく見られるパターンです。データ行の結合セルは
スレーブ側へ broadcast (ADR-0035)。画像・条件付き書式・名前定義・
ウィンドウ枠の固定・シート保護・データの入力規則・セルコメントを扱う
正規の保存マトリックスを追加 (ADR-0036)。そして **`@group` / `@subtotal`**
で、取引先別・月別の小計行を一つのデータブロック内に挟み込めるように
なりました (ADR-0038)。
...
- **Alpha。** XTL は spec 0.1 (draft)。挙動は急速に安定しつつありますが、
  1.0 までは言語表面が変わりうる状態です。
- **メンテナー 1 名。** production reference case はまだありません。xl3 を
  重要な何かに乗せるなら、**現時点で最も価値ある貢献は使ったことを伝えること**
  です — issue や discussion を立ててください。「うちでは動いた」という
  一言だけでも構いません。それが、現場のワークフローにフィットする
  1.0 と、私の想像の中のワークフローにしかフィットしない 1.0 との違いを
  決めます。
- **ADR 70 件、conformance fixture 154 件、すべて green。** 言語表面は
  early adopter が試せる程度には安定しました。
- **MIT、TypeScript、Node 20.12 以上、ブラウザでも動作。**

1.0 のブロッカーは [ROADMAP.md](./ROADMAP.md)、意思決定の進め方は
[GOVERNANCE.md](./GOVERNANCE.md) を参照してください。

> **LLM で xl3 テンプレートを作成していますか?** まず
> [`docs/llm-template-authoring.md`](./docs/llm-template-authoring.md)
> を読んでください — LLM がほぼ確実に繰り返す一つのミス
> (スタイル付きの行が残り、すべての出力に空行として付いてくる問題) と、
> その回避方法を扱っています。リンク先は意図的に英語のままです —
> LLM が直接参照するドキュメントなので、英語で維持しています。
