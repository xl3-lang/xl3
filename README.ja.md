...
xl3 は技術的には安定しつつありますが、プロジェクトとしてはまだ形成期にあります — メンテナー 1 名、production reference case なし、ガバナンスはちょうど文書化されたところです。Audit パスによって silent-fallthrough の挙動はすべて整理され、0.8.0 のデータブロック整備まで含めれば ADR 70 件、fixture 154 件がすべて green です。言語表面は early adopter が試せる程度には安定しました。**現時点で最も価値のある貢献は実際に使ってみたフィードバック**です — 1.0 のブロッカーは [ROADMAP.md](./ROADMAP.md)、意思決定の進め方は [GOVERNANCE.md](./GOVERNANCE.md) を参照してください。

**0.7.0 → 0.8.0 主な変更点** (2026 年 5 月): データブロックが **列スコープ (column-scoped)** 方式に変わりました (ADR-0066)。シート上のすべての `{{ ... }}` マーカーの外接矩形を取り、そこに接する非空セルまで含めた範囲をブロックの列範囲とします。その範囲の外にあるセル — サイド集計表、ヘッダ列、右側のメモなど — は、ブロックが展開しても元の行位置にそのまま残り、行数の増加で押し出されることはありません。これにより、長年の 2 つの不具合が構造的に解消されました — shared-formula owner が重複してセルが静かに消えていた #46、そして横に押し出されたサイドセルの数式参照がズレていた #47。

0.8.0 では、明示的な **`@block`** ディレクティブ (ADR-0067) も追加されました。次の 3 形式で書けます:

- `{{ @block }}` — 引数なし。列範囲はマーカーから自動推定
- `{{ @block A:D }}` — 列範囲を明示
- `{{ @block A2:D7 }}` — 行 × 列の矩形を明示

`@block` を使うシートでは、厳格 (strict) なマルチブロック判定 (ADR-0068) が有効になります — すべての `[Column]` マーカーはいずれかのブロックに属している必要があり、ブロック矩形どうしは重ねられません。他のディレクティブは、最も近くで重なるブロックに自動で結び付けられます (ADR-0069)。**後方互換性:** `@block` がなく、ブロック外側の列にも内容がないテンプレートは 0.7.x と完全に同じ結果を返します。`@block` はオプション (opt-in) 機能です。
