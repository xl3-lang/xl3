...
**0.7.0 → 0.8.0**(2026 年 5 月):資料區塊現在採用**依欄界定
(column-scoped)** 的形態(ADR-0066)。右側的摘要表、欄標題、備註等
在資料區塊展開時保留原本的列位置,修復兩個長期存在的缺陷
(#46 shared-formula owner 重複、#47 被推開的側邊儲存格公式參照失準)。
新增顯式的 **`@block`** 指令(ADR-0067),共三種形態 —— 不帶參數、
欄範圍、列×欄矩形 —— 以及對啟用工作表的嚴格多區塊偵測(ADR-0068)。
**向後相容:** 沒有 `@block`、且區塊外側欄也沒有內容的範本,
渲染結果與 0.7.x 完全一致;`@block` 是可選功能。

**0.6.0 → 0.7.0**:一次 15 個 ADR(ADR-0051..0065)的整合,關閉了所有
殘餘的「同一份範本形狀可能被解讀成兩種,或靜默通過」的語法衝突點。
對使用者影響最明顯的是**聚合函式參數形狀**(ADR-0059):`SUM`、
`AVERAGE`、`MIN`、`MAX` 與 1-arg `COUNT` 現在只接受單一欄位參照,
逐列算式會在解析階段被拒絕。請在來源資料新增輔助欄,或直接在頁尾
儲存格寫原生 `=SUMPRODUCT(...)`(詳見
[Cookbook 03](./docs/guides/03-aggregates.md))。

**0.5.x → 0.6.0**:原生支援合併儲存格標題(ADR-0033)—— 這在
韓國廠商表單(거래명세서 / 정산서 / 발주서)中最為常見,台灣與
華語區廠商表單(對帳單、結算單、採購單)亦有類似格式;資料列的合併
儲存格會把主值 broadcast 到從屬儲存格(ADR-0035);新增涵蓋圖片、
條件式格式設定、名稱定義、凍結窗格、工作表保護、資料驗證與儲存格
註解的正規保留矩陣(ADR-0036);並加入 **`@group` / `@subtotal`**,
可在同一個資料區塊內穿插每客戶 / 每月小計列(ADR-0038)。
...
- **Alpha。** XTL 處於 spec 0.1 (draft)。行為已快速穩定,但語言表面
  在 1.0 前仍可能改變。
- **單一維護者。** 目前還沒有正式上線的參考案例。如果你把 xl3 用在
  重要的地方,現階段**最有價值的貢獻就是告訴我一聲** —— 開個 issue
  或 discussion,即使只是一句「這對我們行得通」也好。這些回饋決定了
  1.0 究竟貼近真實工作流程,還是只貼近我對它的想像。
- **70 個 ADR、154 個 conformance fixture,全部 green。** 語言表面已
  穩定到足以讓 early adopter 嘗試。
- **MIT、TypeScript、Node ≥ 20.12、可在瀏覽器中執行。**

1.0 的 blocker 請見 [ROADMAP.md](./ROADMAP.md);決策方式請見
[GOVERNANCE.md](./GOVERNANCE.md)。

> **正在用 LLM 撰寫 xl3 範本嗎?** 請先閱讀
> [`docs/llm-template-authoring.md`](./docs/llm-template-authoring.md)
> —— 文中說明了 LLM 幾乎每次都會犯的一個錯誤(殘留的樣式列會污染
> 每一份輸出),以及該如何避免。該文件刻意保留英文,因為它是供 LLM
> 直接參照的文件,以英文維持較佳的一致性。
