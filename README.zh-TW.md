...
**0.7.0 → 0.8.0 重點變更**(2026 年 5 月):資料區塊現在採用
**依欄界定(column-scoped)** 的形態(ADR-0066)。引擎會取工作表
上所有 `{{ ... }}` 標記的外接矩形,再向相鄰的非空儲存格延伸,共同
決定資料區塊的欄範圍。該範圍之外的儲存格 —— 側邊摘要表、表頭欄、
右側備註等 —— 在資料區塊展開時保留原本的列位置,不再被列數的
增加推開。這從結構上修復了兩個長期存在的缺陷 —— #46
(shared-formula owner 被重複複製,造成儲存格靜默遺失)與
#47(被橫向推開的側邊儲存格,公式參照失準)。

0.8.0 也新增了顯式的 **`@block`** 指令(ADR-0067),共有三種寫法:

- `{{ @block }}` —— 不帶參數,欄範圍由標記自動推斷
- `{{ @block A:D }}` —— 顯式指定欄範圍
- `{{ @block A2:D7 }}` —— 顯式指定列 × 欄矩形

使用 `@block` 的工作表會啟用嚴格(strict)的多區塊偵測(ADR-0068)
—— 所有 `[Column]` 標記都必須落在某個區塊內,區塊矩形之間不能
重疊。其他指令依鄰近度自動繫結到最近的重疊區塊(ADR-0069)。
**向後相容:** 沒有 `@block`、且區塊外側欄也沒有內容的範本,
渲染結果與 0.7.x 完全一致;`@block` 是可選(opt-in)功能。
